...
if 'info_submitted' not in st.session_state: st.session_state.info_submitted = False
if 'user_name' not in st.session_state: st.session_state.user_name = ""
if 'user_email' not in st.session_state: st.session_state.user_email = ""
if 'fig_substrate_plot' not in st.session_state: st.session_state.fig_substrate_plot = None
if 'fig_target_plot' not in st.session_state: st.session_state.fig_target_plot = None
...
if not st.session_state.info_submitted:
    ...
    # st.success(f"Welcome, {user_display_name}!") # Optional: Can be removed if too intrusive
    ...
    tab1, tab2, tab3 = st.tabs(["Configuration", "Target Data", "Run"])
    ...
    with tab1:
        ...
        with col_config1:
                ...
                if st.session_state.lambda_min_file is not None:
                    ...
                    try:
                        ...
                        if new_config_min < current_max:
                            ...
                            add_log_message("info", f"Substrate changed. Default Min Lambda updated to {st.session_state.config_lambda_min} nm.")
                        else:
            ...
            st.text("Optimization Lambda Range:")
            col_lam1, col_lam2 = st.columns(2)
            with col_lam1:
                st.session_state.config_lambda_min = st.text_input("Min λ (nm):", value=st.session_state.config_lambda_min, key="config_lambda_min_val", help=f"Must be >= substrate limit ({get_substrate_min_lambda(st.session_state.substrate_choice):.1f} nm) and >= file min λ.")
            with col_lam2:
                st.session_state.config_lambda_max = st.text_input("Max λ (nm):", value=st.session_state.config_lambda_max, key="config_lambda_max_val", help="Must be <= file max λ.")
            ...
            with col_thick1a: st.session_state.thickness_min = st.number_input("Min Thick (nm):", min_value=0.0, value=st.session_state.thickness_min, step=10.0, format="%.1f", key="config_thick_min")
            with col_thick2a: st.session_state.thickness_max = st.number_input("Max Thick (nm):", min_value=max(0.1, st.session_state.thickness_min + 0.1), value=st.session_state.thickness_max, step=10.0, format="%.1f", key="config_thick_max")


        with col_config2:
            with st.expander("Advanced Optimization Settings", expanded=True):
    ...
    with tab2:
        ...
        col_target1, col_target2 = st.columns([0.6, 0.4])

        with col_target1:
            st.subheader("Select & Load")
            ...
            if data_source_to_load is not None:
                ...
                try:
                    ...
                    if df is None:
                        raise ValueError(f"Could not read or parse the CSV file '{source_name}' after multiple attempts. Please check format. Errors: {'; '.join(error_messages)}")

                    if df.empty: raise ValueError("CSV file loaded but appears empty.")
                    if 'lambda' not in df.columns or 'target_value' not in df.columns: raise ValueError("Could not find expected columns ('lambda', 'target_value').")
                    ...
                    if rows_after_na < initial_rows:
                        add_log_message("warning", f"{initial_rows - rows_after_na} rows dropped due to non-numeric values.")
                    ...
                    if initial_config_min >= st.session_state.lambda_max_file:
                         add_log_message("warning", f"Min allowed wavelength ({initial_config_min:.1f} nm) for substrate {current_substrate} is >= max file wavelength ({st.session_state.lambda_max_file:.1f} nm). Cannot set default range.")
                         ...
                         add_log_message("info", f"Set default optimization range: [{initial_config_min:.1f}, {st.session_state.lambda_max_file:.1f}] nm")
    ...
    with tab3:
        st.header("Run Optimization & Results")
        ...
        if run_button:
            ...
            try:
                ...
                if not lambda_min_str or not lambda_max_str or lambda_min_str == "---" or lambda_max_str == "---": raise ValueError("Lambda Min/Max not set. Load file and check config.")
                ...
                if st.session_state.lambda_min_file is None or st.session_state.lambda_max_file is None: raise ValueError("File lambda range not available.")
                if effective_lambda_min < st.session_state.lambda_min_file - SMALL_EPSILON or effective_lambda_max > st.session_state.lambda_max_file + SMALL_EPSILON: raise ValueError(f"Optim range [{effective_lambda_min:.1f}, {effective_lambda_max:.1f}] must be within file range [{st.session_state.lambda_min_file:.1f}, {st.session_state.lambda_max_file:.1f}].")
                ...
                if effective_lambda_min < substrate_min_limit:
                     add_log_message("warning", f"Min Lambda ({effective_lambda_min:.1f}) below limit for {selected_substrate} ({substrate_min_limit:.1f}). Adjusting.")
                     effective_lambda_min = substrate_min_limit
                     st.session_state.config_lambda_min = f"{effective_lambda_min:.1f}"
                     if effective_lambda_min >= effective_lambda_max: raise ValueError(f"Adjusted Min Lambda ({effective_lambda_min:.1f}) >= Max Lambda ({effective_lambda_max:.1f}).")
                     st.rerun()
            ...
            if valid_params:
                add_log_message("info", f"Starting: Target={current_target_type}, Substrate={selected_substrate}")
                add_log_message("info", f"Thickness Range: [{thickness_min:.1f}, {thickness_max:.1f}] nm"); add_log_message("info", f"Advanced params: {adv_params}")
                ...
                if not np.any(mask_used_in_optimization):
                    st.error(f"No valid target points found in range [{effective_lambda_min:.1f}, {effective_lambda_max:.1f}] with valid substrate index.", icon="🚨"); add_log_message("error", "No valid points for optimization.")
                else:
                    ...
                    try:
                        ...
                        if len(np.unique(fixed_n_knot_lambdas)) < num_knots_n or len(np.unique(fixed_k_knot_lambdas)) < num_knots_k:
                            add_log_message("warning", "Duplicate knot wavelengths generated. Adjusting slightly.")
                    ...
                    if valid_params:
                        ...
                        def optimization_callback_simple_log(xk, convergence):
                            ...
                            if optim_iteration_count[0] % display_freq == 0 or optim_iteration_count[0] == 1:
                                try:
                                    ...
                                    add_log_message("warning", f"Error in callback iter {optim_iteration_count[0]}: {e_cb}")
                        ...
                        try:
                            ...
                            if not optim_result.success: add_log_message("warning", f"Optim did not converge: {optim_result.message}"); st.warning(f"Optimization warning: {optim_result.message}", icon="⚠️")
                            ...
                            add_log_message("info", "Recalculating spectra & MSE over full range...")
    ...
    # --- Footer Information (outside tabs) ---
    st.divider()
    ...
    with col_foot2:
        ...
        **Steps:**
        1.  **(Optional) Provide Details:** Enter name/email on the welcome screen for logging.
        2.  **Configuration Tab:**
            * Set Substrate Material, Optimization Lambda Range, Thickness Range.
            * (Optional) Adjust Advanced Optimization Settings (knots, DE params).
            * Use "Plot Substrate Indices" or "Reset Parameters" as needed.
        3.  **Target Data Tab:**
            * Select Target Type (T_norm or T_sample). Schema updates.
            * Upload a **.csv** file (Col1: λ (nm), Col2: Target value, header skipped) or use `example.csv`. Delimiter/decimal auto-detected. Target interpreted as fraction or % (auto-divided by 100 if > 5). Plot appears.
        4.  **Run Tab:**
            * Click "▶ Run Optimization". Progress is shown.
            * Analyze Results: Metrics (Thickness, MSE, Quality), Plots (Comparison, n/k), Data Table (expander).

        **Tips:**
        - Check CSV format/logs on loading errors.
        - Use realistic ranges. Lambda range must be valid for substrate & file.
        - Check plot plausibility and fit quality metrics.
        - Set `workers=1` in Advanced Settings if pickling errors occur.
        """
        with st.expander("Help / Instructions", expanded=False):
            st.markdown(help_text_en)
        st.info("Monolayer Optimizer v1.4-tabs - Adapted from original code by F. Lemarchand.")
