...
def calculate_total_error_numba(l_array, nSub_array, target_value_array,
    ...
    for i in range(len(l_array)):
        ...
        elif target_type_flag == 1: calculated_value = max(0.0, min(1.0, Ts_stack))
        else: return HUGE_PENALTY
        ...
        elif not np.isfinite(target_value_array[i]): pass
        else: return HUGE_PENALTY
...
def objective_func_spline_fixed_knots(p, num_knots_n, num_knots_k, l_array, nSub_array, target_value_array, weights_array, target_type_flag, fixed_n_knot_lambdas, fixed_k_knot_lambdas):
    ...
    try:
        ...
        return mse
    except ValueError: return HUGE_PENALTY
    except Exception: return HUGE_PENALTY
...
def plot_target_only(target_data_to_plot, target_filename_base):
    ...
    if np.any(valid_mask):
        ...
        ax_cible.set_ylim(bottom=-5, top=y_lim_top)
...
def create_excel_file(results_data):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        sheet_name_short = results_data['sheet_name']
...
st.title("📈 Simplified Monolayer Optimizer (Streamlit Version)")
...
with st.sidebar:
    ...
    with st.expander("Advanced Optimization Settings"):
        ...
        with col_mut1a: adv_params['mutation_min'] = st.number_input("Mutation Min", min_value=0.0, value=adv_params['mutation_min'], step=0.1)
        with col_mut2a: adv_params['mutation_max'] = st.number_input("Mutation Max", min_value=adv_params['mutation_min'], value=adv_params['mutation_max'], step=0.1)
        ...
        adv_params['workers'] = st.number_input("Parallel Workers (-1 = Auto)", min_value=-1, value=adv_params['workers'], step=1)

col1, col2 = st.columns([0.6, 0.4])

with col1:
    st.subheader("Target Data")
    st.session_state.target_type = st.radio("Select Target Type:", options=["T_norm", "T"], format_func=lambda x: "T Norm (%) = T_sample / T_sub" if x == "T_norm" else "T Sample (%)", index=["T_norm", "T"].index(st.session_state.target_type), horizontal=True, key="target_type_radio")
    uploaded_file = st.file_uploader("Upload Target File (.csv):", type=["csv"], accept_multiple_files=False, key="file_uploader")

    if uploaded_file is not None:
        if uploaded_file.name != st.session_state.get('last_uploaded_filename', None):
            st.session_state.last_uploaded_filename = uploaded_file.name; add_log_message("info", f"Attempting to load: {uploaded_file.name}"); file_extension = os.path.splitext(uploaded_file.name)[1].lower()
            st.session_state.target_data = None; st.session_state.optim_results = None; st.session_state.excel_bytes = None
            try:
                if file_extension == ".csv":
                    add_log_message("info", "Detected .csv format. Using pandas reader."); add_log_message("info", "Expecting comma (,) delimiter and period (.) decimal separator.")
                    try: df = pd.read_csv(uploaded_file, delimiter=',', skiprows=1, usecols=[0, 1], names=['lambda', 'target_value'], encoding='utf-8', skipinitialspace=True); add_log_message("info", "Attempting parse with ',' delimiter, '.' decimal, utf-8 encoding.")
                    except UnicodeDecodeError:
                        add_log_message("info", "UTF-8 decoding failed, trying latin-1..."); uploaded_file.seek(0)
                        try: df = pd.read_csv(uploaded_file, delimiter=',', skiprows=1, usecols=[0, 1], names=['lambda', 'target_value'], encoding='latin-1', skipinitialspace=True ); add_log_message("info", "Successfully parsed with latin-1 encoding.")
                        except Exception as e_latin: raise ValueError(f"Could not read file with latin-1 encoding either. Error: {e_latin}")
                    except Exception as e_pandas:
                        if 'could not convert' in str(e_pandas) and ',' in str(e_pandas):
                            add_log_message("warning", "Initial parse failed. Detected possible comma decimal. Retrying..."); uploaded_file.seek(0)
                            try: df = pd.read_csv(uploaded_file, delimiter=',', decimal=',', skiprows=1, usecols=[0, 1], names=['lambda', 'target_value'], encoding='utf-8', skipinitialspace=True); add_log_message("info", "Successfully parsed CSV with ',' delimiter and ',' decimal (utf-8).")
                            except UnicodeDecodeError:
                                add_log_message("info", "UTF-8 failed on retry, trying latin-1..."); uploaded_file.seek(0); df = pd.read_csv(uploaded_file, delimiter=',', decimal=',', skiprows=1, usecols=[0, 1], names=['lambda', 'target_value'], encoding='latin-1', skipinitialspace=True); add_log_message("info", "Successfully parsed CSV with ',' delimiter, ',' decimal (latin-1).")
                            except Exception as e_retry: raise ValueError(f"Retry with comma decimal failed. Error: {e_retry}")
                        else: raise ValueError(f"Failed to read/parse CSV with pandas (delimiter ','). Error: {e_pandas}")
                    if df is None: raise ValueError("CSV reading failed for unknown reasons.");
                    if df.empty: raise ValueError("CSV file loaded but appears empty after skipping header.")
                    ...
                    if df['lambda'].isnull().any(): add_log_message("warning", f"Non-numeric values found in lambda column (CSV), replaced with NaN.")
                    if df['target_value'].isnull().any(): add_log_message("warning", f"Non-numeric values found in target value column (CSV), replaced with NaN.")
                    data = df.to_numpy(); data = data[data[:, 0].argsort()]; lam = data[:, 0]; target_value_raw = data[:, 1]
                else: raise ValueError(f"Unsupported file extension: '{file_extension}'. Please select a .csv file.")
                if lam is None or target_value_raw is None: raise ValueError("Failed to extract lambda or target data from the file.")
                if np.all(np.isnan(lam)): raise ValueError("Wavelength data contains only non-numeric/empty values.")
                if np.any(np.isnan(lam)): add_log_message("warning", f"Non-numeric lambda values found, processing continues with NaN.")
                ...
                if np.all(np.isnan(target_value_raw)): raise ValueError("Target value data contains only non-numeric/empty values.")
                if np.any(np.isnan(target_value_raw)): add_log_message("warning", f"Non-numeric target values found, processing continues with NaN.")
                valid_targets = target_value_raw[np.isfinite(target_value_raw)]
                if len(valid_targets) > 0 and np.nanmax(valid_targets) > 5: target_value_final = target_value_raw / 100.0; add_log_message("info", f"Target Data ({st.session_state.target_type}) values > 5 detected, interpreting as % and dividing by 100.")
                else: target_value_final = target_value_raw; add_log_message("info", f"Target Data ({st.session_state.target_type}) values <= 5, interpreting as fraction (0-1).")
                st.session_state.target_data = {'lambda': lam, 'target_value': target_value_final, 'target_type': st.session_state.target_type}
                valid_finite_lam = lam[np.isfinite(lam)]
                if len(valid_finite_lam) == 0: raise ValueError("No finite wavelength values found after processing.")
                st.session_state.lambda_min_file = np.min(valid_finite_lam); st.session_state.lambda_max_file = np.max(valid_finite_lam)
                if st.session_state.lambda_min_file >= st.session_state.lambda_max_file - SMALL_EPSILON: raise ValueError("Invalid wavelength range in file after processing.")
                ...
                else: st.session_state.config_lambda_min = f"{initial_config_min:.1f}"; st.session_state.config_lambda_max = f"{st.session_state.lambda_max_file:.1f}"; add_log_message("info", f"Set default optimization range based on file and substrate ({current_substrate}): [{initial_config_min:.1f}, {st.session_state.lambda_max_file:.1f}] nm")
                st.session_state.target_filename_base = uploaded_file.name
                ...
                num_total_pts = len(lam); num_valid_lambda = np.sum(np.isfinite(lam)); num_valid_target = np.sum(np.isfinite(target_value_final))
                add_log_message("info", f"Target Info: '{st.session_state.target_filename_base}' loaded ({num_total_pts} rows)."); add_log_message("info", f"  Valid lambda points: {num_valid_lambda}/{num_total_pts}."); add_log_message("info", f"  Valid target points: {num_valid_target}/{num_total_pts}.")
                ...
                st.error(f"Error reading or processing file '{uploaded_file.name}':\n{e}", icon="🚨"); add_log_message("error", f"ERROR loading file '{uploaded_file.name}': {e}")
    ...
    if st.session_state.target_filename_base:
        if st.session_state.target_filename_base == "Error Loading": st.warning("Error loading the previous file.", icon="⚠️")
        else:
             st.info(f"Loaded: {st.session_state.target_filename_base} (Type: {st.session_state.target_type})", icon="📄")
             if 'fig_target_plot' in st.session_state and st.session_state.fig_target_plot is not None: st.pyplot(st.session_state.fig_target_plot)
    ...
    with col_thick2a: st.session_state.thickness_max = st.number_input("Max Thick (nm):", min_value=st.session_state.thickness_min + 0.1, value=st.session_state.thickness_max, step=10.0, format="%.1f")
...
with col2:
    ...
    if 'fig_substrate_plot' in st.session_state and st.session_state.fig_substrate_plot is not None:
        ...
        if st.button("Clear Substrate Plot"): st.session_state.fig_substrate_plot = None; st.rerun()
...
if run_button:
    ...
    try:
        ...
        if effective_lambda_min < substrate_min_limit:
             add_log_message("warning", f"Specified Min Lambda ({effective_lambda_min:.1f} nm) is below minimum for {selected_substrate} ({substrate_min_limit:.1f} nm). Adjusting...")
    ...
    if valid_params:
        ...
        if not np.any(mask_used_in_optimization): st.error(f"No valid target data points found within the specified lambda range [{effective_lambda_min:.1f}, {effective_lambda_max:.1f}] nm with a valid substrate index.", icon="🚨"); add_log_message("error", "No valid points for optimization in the specified range.")
        else:
            ...
            weights_array = np.ones_like(target_lambda_opt); add_log_message("info", f"Using {len(target_lambda_opt)} target points for optimization.")
            ...
            try:
                ...
                if len(np.unique(fixed_n_knot_lambdas)) < num_knots_n or len(np.unique(fixed_k_knot_lambdas)) < num_knots_k:
                    add_log_message("warning", "Duplicate knot wavelengths generated, likely due to narrow range or low knot count. Adjusting slightly using linspace.")
            ...
            if valid_params:
                ...
                def optimization_callback_simple_log(xk, convergence):
                    ...
                    except Exception: current_fun = np.inf
                    is_best = False
                    if current_fun < optim_callback_best_mse[0]:
                        optim_callback_best_mse[0] = current_fun; is_best = True
                    if is_best or optim_iteration_count[0] % 50 == 0 or optim_iteration_count[0] == 1:
                         mse_val = optim_callback_best_mse[0]
                         status_text.info(f"Iteration: {optim_iteration_count[0]} | Best MSE: {mse_val:.4e}", icon="⏳")
                         add_log_message("info", f"Iter: {optim_iteration_count[0]}, Current MSE: {current_fun:.4e}, Best MSE: {mse_val:.4e}")
                ...
                try:
                    ...
                    valid_lambda_mask_for_calc = (current_target_lambda >= substrate_min_limit) & np.isfinite(current_target_lambda)
                    n_final_array_recalc[valid_lambda_mask_for_calc] = n_spline_final(current_target_lambda[valid_lambda_mask_for_calc]); k_final_array_recalc[valid_lambda_mask_for_calc] = np.exp(log_k_spline_final(current_target_lambda[valid_lambda_mask_for_calc]))
                    n_final_array_recalc = np.clip(n_final_array_recalc, 1.0, N_KNOT_VALUE_BOUNDS[1]); k_final_array_recalc = np.clip(k_final_array_recalc, 0.0, math.exp(LOG_K_KNOT_VALUE_BOUNDS[1]))
                    ...
                    valid_nk_final_mask = np.isfinite(n_final_array_recalc) & np.isfinite(k_final_array_recalc); valid_nsub_mask = np.isfinite(nSub_target_array_recalc); valid_indices_for_T_calc = np.where(valid_nk_final_mask & valid_nsub_mask)[0]
                    ...
                    calc_value_for_mse = T_norm_final_calc if current_target_type == 'T_norm' else T_stack_final_calc; valid_calc_mask = np.isfinite(calc_value_for_mse)
                    combined_valid_mask_for_mse = mask_used_in_optimization & valid_calc_mask; recalc_mse_final = np.nan; percent_good_fit = np.nan; quality_label = "N/A"; mse_pts_count = np.sum(combined_valid_mask_for_mse)
                    if mse_pts_count > 0 :
                        recalc_mse_final = np.mean((calc_value_for_mse[combined_valid_mask_for_mse] - current_target_value[combined_valid_mask_for_mse])**2); abs_delta = np.abs(calc_value_for_mse[combined_valid_mask_for_mse] - current_target_value[combined_valid_mask_for_mse]); delta_threshold = 0.0025
                        points_below_threshold = np.sum(abs_delta < delta_threshold); percent_good_fit = (points_below_threshold / mse_pts_count) * 100.0
                        ...
                        add_log_message("info", f"  Final MSE ({current_target_type}, {mse_pts_count} pts): {recalc_mse_final:.4e}"); add_log_message("info", "-"*20 + " Fit Quality " + "-"*20)
                        add_log_message("info", f"  Range [{effective_lambda_min:.1f}-{effective_lambda_max:.1f}] nm, {mse_pts_count} valid pts"); add_log_message("info", f"  Points with |delta| < 0.25% : {percent_good_fit:.1f}% ({points_below_threshold}/{mse_pts_count})"); add_log_message("info", f"  -> Rating: {quality_label}")
                    else: add_log_message("warning", f"Cannot recalculate Final MSE or Fit Quality for range [{effective_lambda_min:.1f}-{effective_lambda_max:.1f}] nm. No valid points.")
                    add_log_message("info", "-" * 50)
                    st.session_state.optim_results = {'final_spectra': {'l': current_target_lambda, 'T_stack_calc': T_stack_final_calc, 'T_norm_calc': T_norm_final_calc, 'MSE_Optimized': final_mse_display, 'MSE_Recalculated': recalc_mse_final, 'percent_good_fit': percent_good_fit, 'quality_label': quality_label}, 'target_filtered_for_plot': {'lambda': current_target_lambda[combined_valid_mask_for_mse], 'target_value': current_target_value[combined_valid_mask_for_mse], 'target_type': current_target_type}, 'best_params': {'thickness_nm': optimal_thickness_nm, 'num_knots_n': num_knots_n, 'num_knots_k': num_knots_k, 'n_knot_values': n_values_opt_final, 'log_k_knot_values': log_k_values_opt_final, 'n_knot_lambdas': fixed_n_knot_lambdas, 'k_knot_lambdas': fixed_k_knot_lambdas, 'knot_distribution': "1/λ²" if use_inv_lambda_sq_distrib else "1/λ", 'substrate_name': selected_substrate, 'effective_lambda_min': effective_lambda_min, 'effective_lambda_max': effective_lambda_max}, 'plot_lambda_array': np.linspace(effective_lambda_min, effective_lambda_max, 500), 'model_str_base': "Spline Fit", 'excel_export_data': {'lambda (nm)': current_target_lambda, f'n (Spline Fit ({num_knots_n}n{num_knots_k}k))': n_final_array_recalc, f'k (Spline Fit ({num_knots_n}n{num_knots_k}k))': k_final_array_recalc, 'Thickness (nm)': np.full_like(current_target_lambda, optimal_thickness_nm), f'n Substrate ({selected_substrate})': nSub_target_array_recalc, f'Target {current_target_type} (%) (Optimized Range)': np.where(mask_used_in_optimization, current_target_value * 100.0, np.nan), 'Calc T (%)': T_stack_final_calc * 100.0, 'Calc T Norm (%)': T_norm_final_calc * 100.0}, 'excel_summary_params': [('Model', f"Spline Fit ({'1/λ²' if use_inv_lambda_sq_distrib else '1/λ'})"), ('Target Used', current_target_type), ('Substrate', selected_substrate), ('Optimization Lambda Range (nm)', f"[{effective_lambda_min:.1f}, {effective_lambda_max:.1f}]"), ('n Knots', num_knots_n), ('k Knots', num_knots_k), ('Thickness (nm)', f"{optimal_thickness_nm:.3f}"), ('Optim. MSE (Objective)', f"{final_mse_display:.4e}" if np.isfinite(final_mse_display) else "N/A"), ('Final MSE (Recalc. in Range)', f"{recalc_mse_final:.4e}" if np.isfinite(recalc_mse_final) else "N/A"), ('Fit Rating (in Range)', quality_label), ('Pts in Range (|delta|<0.25%)', f"{percent_good_fit:.1f}%" if np.isfinite(percent_good_fit) else "N/A"), ('n Knot λ (nm)', ", ".join([f"{v:.1f}" for v in fixed_n_knot_lambdas])), ('n Knot Values (opti)', ", ".join([f"{v:.4f}" for v in n_values_opt_final])), ('k Knot λ (nm)', ", ".join([f"{v:.1f}" for v in fixed_k_knot_lambdas])), ('k Knot Values (log, opti)', ", ".join([f"{v:.4f}" for v in log_k_values_opt_final]))]}
                    try:
                         excel_data_prep = {'summary': pd.DataFrame(st.session_state.optim_results['excel_summary_params'], columns=['Parameter', 'Value']), 'data': pd.DataFrame(st.session_state.optim_results['excel_export_data']), 'sheet_name': f"Results_{selected_substrate[:10]}"}
                         df_res = excel_data_prep['data']; float_format = "%.4f"
                         for col in df_res.select_dtypes(include=['float']).columns:
                             if col != 'lambda (nm)': df_res[col] = df_res[col].apply(lambda x: float_format % x if pd.notna(x) else '')
...
if st.session_state.optim_results:
    ...
    with col_res2a: mse_disp = results['final_spectra']['MSE_Recalculated']; st.metric("Final MSE (in range)", f"{mse_disp:.4e}" if np.isfinite(mse_disp) else "N/A")
    st.metric("Fit Quality Rating (in range)", results['final_spectra']['quality_label'])
    st.caption(f"Based on {results['final_spectra']['percent_good_fit']:.1f}% points with |Calc - Target| < 0.25%")
    st.subheader("Result Plots")
    fig_compare = plot_spectra_vs_target(res=results['final_spectra'], target=results['target_filtered_for_plot'], best_params_info=results['best_params'], model_str_base=results['model_str_base'], effective_lambda_min=results['best_params']['effective_lambda_min'], effective_lambda_max=results['best_params']['effective_lambda_max'])
    ...
    if st.session_state.excel_bytes:
        st.download_button(label="💾 Download Results (.xlsx)", data=st.session_state.excel_bytes, file_name=st.session_state.excel_filename, mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
    with st.expander("Show Result Data Table"):
        if 'excel_export_data' in results: st.dataframe(pd.DataFrame(results['excel_export_data']))
...
1. Configure Settings (Sidebar):
   ...
   - **Optimization Lambda Range:** Define the Min/Max wavelength (nm) for the optimization. This range must be within the range of your loaded file AND above the substrate's minimum valid wavelength.
   - **Advanced Optimization Settings (Optional):** Expand this section to fine-tune spline parameters (knots) and Differential Evolution algorithm settings (for advanced users).
...
3. Load Target File (Main Area):
   ...
   - **CSV Expected Format:**
     ...
     * **Decimal Separator:** Period (.) - e.g., '93.69'. (The app tries to handle comma decimals too).
     ...
     * **Encoding:** UTF-8 or Latin-1 preferred.
   - *Note:* Ensure your file matches the expected structure. Check for non-numeric values if loading fails.
   - Target values can be percentage (e.g., 95.5) or fraction (e.g., 0.955). The program checks values > 5 and divides by 100 if needed.
   - A plot of the loaded data appears after successful loading.
   - The filename and the valid wavelength range in the sidebar are updated.

4. Configure Thickness Range (Main Area):
   - Define the **Min/Max Thick (nm)** range for the monolayer thickness search.

5. Run Optimization (Main Area):
   - Click the "▶ Run Optimization" button.
   - A spinner indicates the process is running. Progress messages (Iteration/MSE) may appear.
   - The Log Messages section at the bottom provides detailed information.
   - This step performs the search for the best n(λ), k(λ), and thickness 'd'.

6. Analyze Results (Main Area - appears after optimization):
   - **Metrics:** Optimal Thickness, Final MSE, and Fit Quality Rating are displayed.
   - **Result Plots:**
     * Final Comparison Plot: Compares target data (red dots, in range) with the calculated spectrum (blue line). Shows difference ΔT (green).
     * Final n/k Plot: Shows the optimal n(λ) and k(λ) curves and spline knots.
   - **Result Data:**
     * A "Download Results (.xlsx)" button appears to save a detailed Excel file.
     * An expander allows viewing the result data table directly in the app.

Tips:
- Ensure the CSV file format is correct (comma separator, period decimal, 2 columns after header).
- Choose a realistic thickness range.
...
- A low MSE value and a good 'Fit Quality' indicate a successful fit.
- Visually inspect the plots to assess the physical relevance of the resulting n(λ) and k(λ).
"""
...
st.sidebar.markdown("---")
st.sidebar.info("Streamlit App Version based on the original Monolayer Optimizer.")
