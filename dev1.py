# -*- coding: utf-8 -*- # Necessary for special characters in comments/strings
import logging
...
# --- Optical Calculation Functions (Numba JIT) ---
...
def get_n_substrate(substrate_id, wavelength_nm):
    ...
    l_um_sq = l_um * l_um
    # ... (the rest of the get_n_substrate function remains unchanged)
    if substrate_id == 0: # SiO2
        ...
        min_wl = 230.0
        if wavelength_nm < min_wl: return np.nan
        # Coefficients for Sapphire (ordinary, various sources, check if adequate)
        # Coefficients are missing here, using SiO2 as fallback, correct if necessary!
        # B1 = 1.43134930; C1 = 0.0052799261; B2 = 0.65054713; C2 = 0.014238474; B3 = 5.3414021; C3 = 325.01783
        # Using SiO2 until correction
        B1=0.6961663; C1=0.0684043**2; B2=0.4079426; C2=0.1162414**2; B3=0.8974794; C3=9.896161**2
        return sellmeier_calc(l_um_sq, B1, C1, B2, C2, B3, C3)
        ...
        return sellmeier_calc(l_um_sq, B1, C1, B2, C2, B3, C3)
    else: # Fallback to SiO2 if ID unknown
        min_wl = 230.0
        if wavelength_nm < min_wl: return np.nan
        B1=0.6961663; C1=0.0684043**2; B2=0.4079426; C2=0.1162414**2; B3=0.8974794; C3=9.896161**2
        return sellmeier_calc(l_um_sq, B1, C1, B2, C2, B3, C3)
...
def calculate_monolayer_lambda(l_val, nMono, thickness_nm, nSub_val):
    # ... (the rest of the calculate_monolayer_lambda function remains unchanged)
    ...
    # Interface Substrate -> Output Medium (Air)
    ...
    # Characteristic matrix of the monolayer
    ...
    # Calculation of reflection/transmission coefficients of the complete structure
    ...
    # Incoherent combination (thick substrate)
    ...
    R_prime_global_calc = Rb_plus + (Tb_plus * Ra_minus * Tb_minus) / denom_global_bwd # Reflection on the substrate side
...
def calculate_total_error_numba(l_array, nSub_array, target_value_array,
    ...
    # ... (the rest of the calculate_total_error_numba function remains unchanged)
    total_sq_error = 0.0
    points_calculated = 0
    for i in range(len(l_array)):
        l_val = l_array[i]
        nSub_val = nSub_array[i]
        # Ignore the point if the substrate is not defined at this lambda
        if not np.isfinite(nSub_val): continue

        n_calc = n_calc_array[i]
        k_calc = k_calc_array[i]

        # Force minimum physical bounds
        n_calc = max(1.0, n_calc) # n cannot be < 1
        k_calc = max(0.0, k_calc) # k cannot be < 0

        # Penalty if calculated n/k values are outside the optimizer bounds
        if not (n_min_bound <= n_calc <= n_max_bound): return HUGE_PENALTY
        if not (k_min_bound <= k_calc <= k_max_bound): return HUGE_PENALTY
        if not (np.isfinite(n_calc) and np.isfinite(k_calc)): return HUGE_PENALTY # Penalty if NaN/inf

        nMono_complex_val = n_calc - 1j * k_calc

        # Calculate the stack transmission
        _, Ts_stack, _ = calculate_monolayer_lambda(l_val, nMono_complex_val, current_thickness_nm, nSub_val)
        if not np.isfinite(Ts_stack): continue # Ignore if optical calculation fails

        calculated_value = np.nan
        if target_type_flag == 0: # T_norm
            # Calculate the transmission of the bare substrate
            _, Ts_sub, _ = calculate_monolayer_lambda(l_val, 1.0 + 0j, 0.0, nSub_val) # n=1, k=0, d=0
            if not np.isfinite(Ts_sub): continue # Ignore if optical calculation fails

            # Calculate T_norm
            if Ts_sub > SMALL_EPSILON: T_norm_calc = Ts_stack / Ts_sub
            else: T_norm_calc = 0.0 if abs(Ts_stack) < SMALL_EPSILON else HUGE_PENALTY

            if not np.isfinite(T_norm_calc): return HUGE_PENALTY # Penalty if NaN/inf
            calculated_value = max(0.0, min(2.0, T_norm_calc)) # T_norm can be > 1, but limit reasonably
        ...
        # Accumulate squared error if target is valid
        if np.isfinite(target_value_array[i]) and np.isfinite(calculated_value):
            ...
            points_calculated += weights_array[i]
             ...
             pass # Ignore points where target is NaN
        else: # If calculated_value is NaN but target is not -> penalty
            return HUGE_PENALTY

    # Return mean squared error
    if points_calculated <= SMALL_EPSILON: return HUGE_PENALTY # Avoid division by zero
...
def objective_func_spline_fixed_knots(p, num_knots_n, num_knots_k, l_array, nSub_array, target_value_array, weights_array, target_type_flag, fixed_n_knot_lambdas, fixed_k_knot_lambdas):
    # ... (the rest of the objective_func_spline_fixed_knots function remains unchanged)
    expected_len = 1 + num_knots_n + num_knots_k
    if len(p) != expected_len: return HUGE_PENALTY # Basic check
    ...
    if current_thickness_nm < 0: return HUGE_PENALTY # Negative thickness
    ...
    try:
        ...
        if num_knots_n < 2 or num_knots_k < 2 : return HUGE_PENALTY # Spline needs at least 2 points
    ...
    except ValueError: # Potential CubicSpline error (e.g., non-finite values, unsorted)
        return HUGE_PENALTY
    except Exception: # Other unexpected errors
        return HUGE_PENALTY

# --- Logging and Streamlit Display Functions ---
...
def plot_target_only(target_data_to_plot, target_filename_base):
    # ... (the rest of the plot_target_only function remains unchanged)
    ...
    # X-axis limits based on file data or stored values
...
def plot_nk_final(best_params_info, plot_lambda_array):
    # ... (the rest of the plot_nk_final function remains unchanged)
    fig, ax1 = plt.subplots(1, 1, figsize=(7, 5))
    ax2 = ax1.twinx() # Shared Y-axis for n and k
    ...
    try:
        ...
        color1='tab:red'; color2='tab:blue' # Colors for k and n
        ...
        # Y-scale for K (log or linear depending on range)
...
def plot_spectra_vs_target(res, target=None, best_params_info=None, model_str_base="Spline Fit", effective_lambda_min=None, effective_lambda_max=None):
    # ... (the rest of the plot_spectra_vs_target function remains unchanged)
    try:
        fig, ax = plt.subplots(1, 1, figsize=(9, 7)); ax_delta = ax.twinx() # Axis for difference

        # Building title and labels
        ...
        # Plot difference (Calculated - Target) on the right axis
        line_delta = None; delta_t_perc = np.full_like(calc_l, np.nan) if calc_l is not None else np.array([])
        if calc_l is not None and calc_y is not None and target_l_valid is not None and target_y_valid is not None and len(target_l_valid) > 1: # Need enough target points to interpolate
            calc_y_perc = calc_y * 100.0
            # Interpolate target onto calculated wavelengths to subtract
        ...
        if np.any(plot_mask_delta):
            ...
            # If no delta calculable, configure axis anyway but without data/limits
        ...
        else: # Fallback if effective range is not defined
        ...
        # Display fit quality on the graph
        ...
        if np.isfinite(percent_good_fit):
            ...
            # Position text at bottom right of the graph
...
def plot_substrate_indices():
    # ... (the rest of the plot_substrate_indices function remains unchanged)
    fig, ax = plt.subplots(1, 1, figsize=(8, 6))
    lambda_nm = np.linspace(200, 2000, 500) # Wavelength range for plotting
    ...
    for idx, name in enumerate(SUBSTRATE_LIST):
        ...
        try:
            ...
            # Plot the valid part with solid line
            ...
            # Plot the invalid part (extrapolation or out of range) with dotted line
            invalid_mask = lambda_nm < min_wl_sub
            if np.any(invalid_mask):
                # Recalculate for the invalid part to avoid errors if get_n_substrate returns NaN
    ...
    ax.minorticks_on()
    ax.legend()
    ax.set_xlim(200, 2000) # Fixed limits for comparison
    plt.tight_layout()
    return fig

def draw_schema_matplotlib(target_type, substrate_name):
    # ... (the rest of the draw_schema_matplotlib function remains unchanged)
    fig, ax = plt.subplots(figsize=(5.5, 1.5)) # Adjust size as needed
    ...
    # Draw layer and substrate (left or single if T_sample)
    ...
    # Arrow and label for transmission (T_sample or T_norm)
    ...
    # Additional section for T_norm (bare substrate on the right)
    if target_type == 'T_norm':
        x_right = 100 - 10 - stack_width # Position on the right
        ...
        # Explanatory text for T_norm in the center
        x_center = 50
        text_tnorm = f"{label_text} = T_sample / T_sub"
        # text_tnorm = "considering this\nautozero/baseline\nconfiguration" # Original text, maybe less clear
...
def create_excel_file(results_data):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # Use a short and informative sheet name
        sheet_name_short = results_data.get('sheet_name', 'Results')[:30] # Limit sheet name length

        # Write summary first
        df_summary.to_excel(writer, index=False, sheet_name=sheet_name_short, startrow=0, startcol=0)

        # Write detailed data after the summary, with a space
...
# Use st.session_state to preserve values between re-runs
...
if 'last_loaded_source' not in st.session_state: st.session_state.last_loaded_source = None # To track if default or upload
...
st.set_page_config(page_title="Monolayer Optimizer", layout="wide") # Tab name

# --- Title and Integrated Explanation ---
st.title("Monolayer Optical Properties Optimizer")

with st.expander("📄 Application Description", expanded=False): # 'expanded=False' so it's closed by default
    st.markdown(r"""
    ...
    The tool outputs the determined dispersions of $n(\lambda)$ and $k(\lambda)$, the optimal thickness $d$, comparison plots, an assessment of the fit quality, and allows exporting detailed results to an Excel file.
...
with st.sidebar:
    ...
    # Auto-update min lambda if the substrate changes AND a file is loaded
    if st.session_state.substrate_choice != prev_substrate and st.session_state.target_data is not None:
        ...
        if st.session_state.lambda_min_file is not None:
            ...
            except ValueError: # In case config_lambda_max is not a valid float
               st.session_state.config_lambda_min = f"{new_config_min:.1f}"
               add_log_message("info", f"Substrate changed to {st.session_state.substrate_choice}. Default Min Lambda updated to {st.session_state.config_lambda_min} nm.")
            st.rerun() # Updates the sidebar display
    ...
    # Simple validation of lambda inputs
    try:
    ...
    # Button to display substrate index plot
...
# --- Main Columns ---
col1, col2 = st.columns([0.6, 0.4]) # 60% for controls/plots, 40% for schema/substrate
...
with col1:
    ...
    data_source_to_load = None # Will contain either uploaded_file or default_file_path
    source_name = None         # Name of the file (uploaded or default)
    is_default_file = False

    if uploaded_file is not None:
        # Priority to user-uploaded file
        if uploaded_file.name != st.session_state.get('last_loaded_source', None):
            data_source_to_load = uploaded_file
            source_name = uploaded_file.name
            st.session_state.last_loaded_source = source_name # Remember the loaded file name
            add_log_message("info", f"User uploaded file: {source_name}")
            reset_log() # Clear previous logs on new load
            st.session_state.optim_results = None # Reset previous results
            st.session_state.excel_bytes = None
    elif st.session_state.target_data is None: # If no file uploaded AND no data already loaded
        # Try to load the default file ONLY if no data is present
        ...
        if os.path.exists(default_file_path):
            if default_file_path != st.session_state.get('last_loaded_source', None):
                ...
                reset_log() # Clear previous logs
                st.session_state.optim_results = None # Reset previous results
                st.session_state.excel_bytes = None
            # else: Default file already loaded and remembered, do nothing
        else:
            # Default file not found and no upload -> Display message later
            if st.session_state.last_loaded_source != "DEFAULT_NOT_FOUND": # Avoid repeating message
    ...
    # --- Parsing logic (common for upload and default) ---
    if data_source_to_load is not None:
        st.session_state.target_data = None # Reset before parsing
        try:
            ...
            # CSV read attempts (adapted from your original code)
            parse_attempts = [
                ...
                {'delimiter': ',', 'decimal': ',', 'encoding': 'utf-8', 'msg': "Parse attempt: Delimiter=',' Decimal=',' Encoding='utf-8'"},
                {'delimiter': ',', 'decimal': ',', 'encoding': 'latin-1', 'msg': "Parse attempt: Delimiter=',' Decimal=',' Encoding='latin-1'"},
                # Add other combinations if necessary (e.g., semicolon)
                # {'delimiter': ';', 'decimal': ',', 'encoding': 'utf-8', 'msg': "Parse attempt: Delimiter=';' Decimal=',' Encoding='utf-8'"},
                # {'delimiter': ';', 'decimal': '.', 'encoding': 'utf-8', 'msg': "Parse attempt: Delimiter=';' Decimal='.' Encoding='utf-8'"},
            ]
            ...
            for attempt in parse_attempts:
                try:
                    # Use data_source_to_load which is either UploadedFile or a path
                    ...
                    df_attempt = pd.read_csv(
                        ...
                        usecols=[0, 1],     # Use only the first 2 columns
                        names=['lambda', 'target_value'], # Name the columns
                        encoding=attempt['encoding'],
                        skipinitialspace=True # Ignore spaces after delimiter
                    )
                    # Simple check if parsing worked (at least one row and expected columns)
            ...
            # Convert % -> fraction if necessary
            valid_targets = target_value_raw[valid_target_mask]
            if len(valid_targets) > 0 and np.nanmax(valid_targets) > 5: # Arbitrary threshold to detect %
            ...
            st.session_state.target_filename_base = source_name # Remember the used file name

            # Update default lambda range in sidebar
            ...
            st.rerun() # Refresh the interface with new data and lambda range
            ...
            st.session_state.last_loaded_source = f"ERROR_{source_name}" # Avoid re-attempting failed load
            st.rerun() # Refresh to show the error


    # Display loaded file status (or error) and target plot if available
    if st.session_state.target_filename_base:
        if st.session_state.target_filename_base == "Error Loading":
            # Error is already displayed by the try/except block
            pass
             ...
             else: # In case plot couldn't be generated but data is there
                 st.warning("Target data loaded, but plot could not be generated.", icon="⚠️")
    elif st.session_state.last_loaded_source != "DEFAULT_NOT_FOUND": # If no file loaded and no default file error
...
with col2:
    ...
    # Schema updates automatically thanks to st.radio and st.selectbox triggering a rerun
    fig_schema = draw_schema_matplotlib(st.session_state.target_type, st.session_state.substrate_choice)
    st.pyplot(fig_schema)

    # Display substrate plot if generated
...
if run_button:
    ...
    try:
        ...
        if effective_lambda_min < substrate_min_limit:
              ...
              effective_lambda_min = substrate_min_limit; st.session_state.config_lambda_min = f"{effective_lambda_min:.1f}" # Update session state too
              ...
              st.rerun() # Important to display adjusted value before continuing optimization
    ...
    if valid_params:
        ...
        # Filter target data for optimization
        ...
        if not np.any(mask_used_in_optimization):
            ...
            weights_array = np.ones_like(target_lambda_opt); # Uniform weights for now
            add_log_message("info", f"Using {len(target_lambda_opt)} target points for optimization.")

            # Calculate fixed knot positions for splines
            ...
            try:
                if use_inv_lambda_sq_distrib:
                    ...
                    # Distribution in 1/lambda (linear in frequency/energy)
                ...
                # Ensure knots are within range and sorted (clip + sort)
                ...
                # Check for duplicates (can happen if range is very narrow)
            ...
            if valid_params:
                ...
                # Fixed arguments for the objective function
                ...
                status_text = st.empty() # Placeholder to display progress

                def optimization_callback_simple_log(xk, convergence):
                    optim_iteration_count[0] += 1
                    # Evaluate current objective (can be expensive, do sparingly)
                    # Display every N iterations or if better MSE found
                    display_freq = 50
                    is_best = False
                    if optim_iteration_count[0] % display_freq == 0 or optim_iteration_count[0] == 1:
                        try:
                             ...
                             # More detailed logging in the log expander
                ...
                de_args = {
                    ...
                    'disp': False, # Do not display scipy output in console
                ...
                try:
                    ...
                    # Recalculate n, k, T_stack, T_norm over the ENTIRE lambda range of the original file
                    ...
                    nSub_target_array_recalc = np.array([get_n_substrate(substrate_id, l) for l in current_target_lambda]) # nSub over full range
                    ...
                    # Mask to evaluate splines only where lambda is valid
                    ...
                    if np.any(valid_lambda_mask_for_calc):
                         ...
                         # Clip to physical/optimizer bounds after potential extrapolation
                    ...
                    # Calculate Tstack and Tnorm where n, k, and nSub are valid
                    ...
                    for i in valid_indices_for_T_calc:
                        ...
                        try:
                            _, Ts_stack_calc, _ = calculate_monolayer_lambda(l_val, nMono_val, optimal_thickness_nm, nSub_val); _, Ts_sub_calc, _ = calculate_monolayer_lambda(l_val, 1.0 + 0j, 0.0, nSub_val) # Bare substrate
                            ...
                            if np.isfinite(Ts_sub_calc):
                                ...
                                elif abs(Ts_stack_calc) < SMALL_EPSILON : T_norm_calc = 0.0 # If both are close to 0
                            if np.isfinite(T_norm_calc): T_norm_final_calc[i] = np.clip(T_norm_calc, 0.0, 2.0) # Tnorm can exceed 1
                        except Exception: pass # Ignore isolated calculation errors

                    # Recalculate final MSE and quality WITHIN THE OPTIMIZATION RANGE
                    ...
                    combined_valid_mask_for_mse = mask_used_in_optimization & valid_calc_mask_recalc; # Use original optimization mask
                    ...
                    if mse_pts_count > 0 :
                        ...
                        # Calculate fit quality (percentage of points with delta < threshold)
                        abs_delta = np.abs(calc_value_for_mse[combined_valid_mask_for_mse] - current_target_value[combined_valid_mask_for_mse]); delta_threshold = 0.0025 # 0.25% threshold
                    ...
                    # Store all results for display and export
                    plot_lambda_array_final = np.linspace(effective_lambda_min, effective_lambda_max, 500) # For plotting n/k
                    st.session_state.optim_results = {
                        'final_spectra': {
                            'l': current_target_lambda, # Lambda over full file range
                            'T_stack_calc': T_stack_final_calc, # Calculation over full range
                            'T_norm_calc': T_norm_final_calc,   # Calculation over full range
                            'MSE_Optimized': final_mse_display,
                            'MSE_Recalculated': recalc_mse_final, # MSE in optim range
                            'percent_good_fit': percent_good_fit, # Quality in optim range
                            'quality_label': quality_label      # Label in optim range
                        },
                        'target_filtered_for_plot': { # Target only in optim range for the plot
                        ...
                        'best_params': {
                            ...
                            'effective_lambda_min': effective_lambda_min, # Range actually used
                            'effective_lambda_max': effective_lambda_max
                        },
                        'plot_lambda_array': plot_lambda_array_final, # Lambda for plotting n/k
                        'model_str_base': "Spline Fit",
                        # Data for Excel export (over full file range)
                        'excel_export_data': {
                            ...
                            # Show target only in optim range in Excel for clarity
                            f'Target {current_target_type} (%) (Used)': np.where(mask_used_in_optimization, current_target_value * 100.0, np.nan),
                            f'Target {current_target_type} (%) (Full File)': current_target_value * 100.0, # New column
                            'Calc T (%)': T_stack_final_calc * 100.0,
                            'Calc T Norm (%)': T_norm_final_calc * 100.0,
                            'Delta T (%)': (T_stack_final_calc - current_target_value)*100.0 if current_target_type == 'T' else np.nan, # Added Delta
                            'Delta T Norm (%)': (T_norm_final_calc - current_target_value)*100.0 if current_target_type == 'T_norm' else np.nan, # Added Delta
                        },
                        'excel_summary_params': [
                            ...
                            # Knot details (optional, can be long)
                    ...
                    try:
                        excel_data_prep = {
                            ...
                            'sheet_name': f"Results_{selected_substrate[:10]}" # Short sheet name
                        }
                        # Format numbers in the data DataFrame
                        df_res = excel_data_prep['data']; float_format = "%.5f" # More decimals for n/k/T
                        ...
                        # More descriptive Excel filename
                ...
                except Exception as e_optim: st.error(f"Optimization Error: An error occurred during optimization:\n{e_optim}", icon="🚨"); add_log_message("error", f"ERROR during optimization: {e_optim}"); traceback.print_exc() # Print trace for debugging
...
if st.session_state.optim_results:
    ...
    # Pass filtered target for comparison plot
    fig_compare = plot_spectra_vs_target(
        res=results['final_spectra'],
        target=st.session_state.target_data, # Pass all original target data
        # target=results['target_filtered_for_plot'], # <- Old version: only used points
    ...
    with st.expander("Show Result Data Table (Full Range)"):
        if 'excel_export_data' in results:
            # Display the DataFrame prepared for Excel (already formatted)
